  ...
  int AdjustSize(int iNum)
  {
    if(iNum > 0)
    {
      ...
      for(int _i=0; _i<iNum; _i++)
      {
        ...
        // set priority
        SetThreadPriority(pNew->_Handle, THREAD_PRIORITY_ABOVE_NORMAL);
        assert(pNew->_Handle);
      }
...
protected:
  ...
  static DWORD WINAPI DefaultJobProc(LPVOID lpParameter = NULL)
  {
    ...
    for(;;)
    {
	  ...
	  printf("EnterCriticalSection(&pThreadPoolObj->_csWorkQueue)\n");
      if(fHasJob = !pThreadPoolObj->_JobQueue.empty())
      {
		printf("thread fHasJob=%d\n",fHasJob);
        pJob = pThreadPoolObj->_JobQueue.front();
        pThreadPoolObj->_JobQueue.pop();
        assert(pJob);
      }
      LeaveCriticalSection(&pThreadPoolObj->_csWorkQueue);
	  printf("leaveCriticalSection(&pThreadPoolObj->_csWorkQueue)\n");
